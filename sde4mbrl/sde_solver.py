...
def euler_maruyama_solver_uarr(time_step, x0, us, rng_brownian, drift_fn, diffusion_fn, 
                                projection_fn=None, extra_scan_args=None):
    """Implement Euler-Maruyama method for Ito SDEs -> u is given as an array
    ...
    carry_init = x0
    xs = (dw, us, time_step, time_step) if extra_scan_args is None else (dw, us, time_step, extra_scan_args)
    _, yevol = jax.lax.scan(euler_step, carry_init, xs)
    return jnp.concatenate((x0[None], yevol))


def euler_maruyama_solver_ufun(time_step, x0, us, rng_brownian, drift_fn, diffusion_fn, 
                                projection_fn=None, extra_scan_args=None):
    """Implement Euler-Maruyama method for Ito SDEs -> u is given as a function
    ...
    carry_init = x0
    xs = (dw, time_step, time_step) if extra_scan_args is None else (dw, time_step, extra_scan_args)
    _, (yevol, uevol) = jax.lax.scan(euler_step, carry_init, xs)
    return jnp.concatenate((x0[None], yevol)), uevol


def euler_maruyama(time_step, x0, us, rng_brownian, drift_fn, diffusion_fn, 
                    projection_fn=None, extra_scan_args=None):
    """ A wrapper function for both the case where us is a function or
        a set of control inputs
    """
    solfn = euler_maruyama_solver_uarr if hasattr(us, 'ndim') else euler_maruyama_solver_ufun
    return solfn(time_step, x0, us, rng_brownian, drift_fn, diffusion_fn, projection_fn, extra_scan_args)
