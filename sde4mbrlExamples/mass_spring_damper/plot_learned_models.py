...
from mass_spring_model import load_data_generator, load_learned_model, _load_pkl
...
xlb_range = np.array([-0.15, -0.15])
xub_range = np.array([0.15, 0.15])
# Random PRNG number
seed = 0
...
def generate_data_for_plots(outfile='my_data'):
    ...
    for _k, model_name in enumerate(my_models_name):
        ...
        train_data = np.array([ _x for (xev, _) in train_data for _x in xev])
...
data2show = ['node_bboxes', 'groundtruth', 'nesde_bboxes', 'nesde_phys'] #, 'nesde_bboxes', 'groundtruth']
for dataname, _data in data4comparison.items():
