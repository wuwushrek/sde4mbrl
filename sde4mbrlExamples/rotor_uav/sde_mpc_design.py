...
def one_step_cost_f(x, u, extra_args, _cost_params):
    ...
    qerr = jnp.sum(jnp.square(quatmult(x[6:10], quatinv(xref[6:10], jnp), jnp)[1:]) * jnp.array(_cost_params['qerr']))
    # Compute the error on the rest of the states
    perr = jnp.sum(jnp.square(x[:6] - xref[:6]) * jnp.array(_cost_params['perr']))
    # Compute the error on the angular velocity
    werr = jnp.sum(jnp.square(x[10:] - xref[10:]) * jnp.array(_cost_params['werr']))
    # Compute the cost
    uerr = jnp.sum(jnp.square(u - jnp.array(_cost_params['uref'])) * jnp.array(_cost_params['uerr']))
    return perr + qerr + werr + uerr

def augmented_cost(u_params, _multi_sampling_cost, _cost_params, _time_steps):
...
def apg_mpc(xt, rng, past_solution, target_x, multi_cost_sampling, proximal_fn, cfg_dict, 
    ...
    cost_xt = lambda u_params: augmented_cost(u_params, _cost_xt, cfg_dict['cost_params'], _timesteps_arr)
...
def load_mpc_from_cfgfile(path_config, convert_to_enu=True, nominal_model=False):
    ...
    _one_step_function = load_predictor_function(cfg_dict['learned_model_params'], 
                        modified_params={'horizon' : 1, 'num_particles' : 1}, prior_dist=nominal_model)
